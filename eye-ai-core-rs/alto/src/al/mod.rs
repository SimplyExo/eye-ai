...
pub unsafe trait Source {
	...
	fn set_sample_offset(&mut self, _: sys::ALint) -> AltoResult<()>;

	/// `alGetSourcei(AL_BYTE_OFFSET)`
	fn byte_offset(&self) -> sys::ALint;
	/// `alSourcei(AL_BYTE_OFFSET)`
	fn set_byte_offset(&mut self, _: sys::ALint) -> AltoResult<()>;
...
impl Context {
	...
	pub(crate) fn make_current(&self, set: bool) -> Option<MutexGuard<'_, ()>> {
		Context::make_raw_current(&self.0.dev, if set { self.0.ctx } else { ptr::null_mut() })
	}

	fn make_raw_current(dev: &Device, ctx: *mut sys::ALCcontext) -> Option<MutexGuard<'_, ()>> {
...
impl<'c> DeferLock<'c> {
	fn new(ctx: &'c Context) -> DeferLock<'c> {
		let _ = (|| -> AltoResult<_> {
...
impl StaticSource {
	pub(crate) fn new(ctx: Context) -> AltoResult<StaticSource> {
		...
		let sends = iter::repeat_n(0, ctx.0.dev.0.max_aux_sends() as usize).collect();
		Ok(StaticSource {
