# train_nlp_qnn_cnn_v3_compact.py - Kompaktes NPU-Training (weniger RAM)
...
VOCAB_SIZE = 10000
SEQUENCE_LENGTH = 250
BATCH_SIZE = 32  # Reduziert für weniger RAM
...
# -----------------------------
# One-Hot Encoding (Lazy, um RAM zu sparen)
# -----------------------------
def preprocess_for_model(sequences):
    return tf.one_hot(sequences, VOCAB_SIZE, axis=-1)

# Trainingsdaten mit Lazy One-Hot (ohne Batch-Prefetch)
...
# -----------------------------
# Kompakteres CNN-Modell (weniger Parameter)
# -----------------------------
inputs = tf.keras.Input(shape=(SEQUENCE_LENGTH, VOCAB_SIZE), dtype=tf.float32, name="input_onehot")

# Reduzierte Filteranzahl
x = tf.keras.layers.Conv1D(128, 5, activation='relu', padding='same', name="conv1")(inputs)
...
model = tf.keras.Model(inputs, outputs, name="nlp_classifier_cnn_v3_compact")
...
print("\n=== Training gestartet (max 30 Epochen) ===")
model.fit(
    train_onehot,
    epochs=30,
...
# Keras Modell speichern (für uint8 Quantisierung)
model.save("nlp_model_v3.keras")
print("✅ Keras Modell gespeichert (nlp_model_v3.keras)")
...
# Full Integer (uint8 input, float32 output for softmax)
