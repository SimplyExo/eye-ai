# compare_models_v2.py - Modellvergleich der vorhandenen Modelle
...
VOCAB_SIZE = 5000  # v2 verwendet 5000
SEQUENCE_LENGTH = 100  # v2 verwendet 100

# Modelle definieren
MODELS = {
    "Alt (LSTM)": "/home/robert/Dokumente/GitHub/eye-ai-clone/eye-ai/NLPTraining/nlp_model.tflite",
    "Neu v2 Dynamic": "nlp_model_onehot_dynamic.tflite",
    "Neu v2 Float16": "nlp_model_onehot_float16.tflite",
}
...
# V2 hat 5000 Vokabular,修剪 auf 5000 falls nötig
        ...
        for i in range(num_samples):
            try:
                ...
                if "LSTM" in model_name:
                    # Altes LSTM: String Input (mit shaped array)
                    text = val_texts_for_old[i]
                ...
                continue  # Skip samples mit Input-Problemen
...
# -----------------------------
print("\n" + "="*80)
print(" " * 25 + "MODELLVERGLEICH - ZUSAMMENFASSUNG")
print("="*80)

if results:
    print(f"\n{'Modell':<25} {'Größe':<10} {'Accuracy':<12} {'Latenz':<15} {'NPU':<8}")
    print("-" * 80)

    for name, res in results.items():
        npu_icon = "✅" if res['npu_compatible'] else "❌"
        print(f"{name:<25} {res['size_mb']:<10.2f} {res['accuracy']:<12.2f} {res['avg_latency_ms']:<15.2f} {npu_icon:<8}")
    ...
    if npu_models:
        ...
        print(f"   - Gewichte: int8 quantisiert (Dynamic Range)")
        print(f"   - Input: {best_npu_all[1]['input_dtype']}")
        print(f"   - Output: {best_npu_all[1]['output_dtype']}")
    else:
        print("⚠️  Kein NPU-kompatibles Modell verfügbar!")

print("\n📝 Hinweise:")
print("   - Dynamic Range Quantization hat bereits int8 Gewichte")
print("   - Für uint8 ist ein Keras Modell nötig (für representative dataset)")
print("   - Dynamic ist auf NPU oft genauso gut wie uint8")