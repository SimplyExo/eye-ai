...
AudioData::AudioData(float frequency, float duration) {
	...
	this->numSamples = static_cast<int>(sampleRate * duration);
	this->samples = getAudioTone();
}
...
std::vector<short> AudioData::getAudioTone() {
	...
	= 44100 => t = 44100 / 44100 = 1s std::sin(2.0f * PI * frequency * t):
	mathematical sin-function: sin(2πft) adapted amplitude * 32760: sin
