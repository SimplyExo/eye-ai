...
std::vector<std::vector<float>> reshape(std::vector<float> input, unsigned int values, unsigned int detections)
{
    std::vector<std::vector<float>> output(detections, std::vector<float>(values));
...
int main() {
    ...
    for (int i = 0; i < 8400; ++i) {
        ...
        if (maxConf > confidenceThreshold) {
            ...
            left = std::max(0, left);
            top = std::max(0, top);
            if (left + width > INPUT_WIDTH) width = INPUT_WIDTH - left;
            if (top + height > INPUT_HEIGHT) height = INPUT_HEIGHT - top;

            boxes.emplace_back(left, top, width, height);
            confidences.push_back(maxConf);
            classIds.push_back(classId);

            std::cout << "Box: (" << left << "," << top << ")-(" << (left+width) << "," << (top+height)
                      << ") Klasse: " << coco_labels[classId] << " Konfidenz: " << maxConf << std::endl;
        }
    }
    ...
    for (int idx : indices) {
        ...
        std::string label = coco_labels[classIds[idx]] + ": " + cv::format("%.2f", confidences[idx]);
        int baseLine;
