...
class SpecializedSettingsTfliteRuntimeTest {
	...
	fun `head routing is word to operation and character to speaker only`() {
		...
		val predictors =
			SpecializedSettingsTflitePredictors(
				wordTokenizer = FrozenSettingsTokenizer(
					FrozenSettingsTokenizer.SPECIAL_TOKENS.withIndex()
		...
		assertArrayEquals(
			floatArrayOf(0.01f, 0.90f, 0.02f, 0.03f, 0.04f), operation.probabilities, 0f
		)
	}
