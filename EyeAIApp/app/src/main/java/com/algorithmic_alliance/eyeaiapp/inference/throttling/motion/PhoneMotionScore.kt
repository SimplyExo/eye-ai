...
data class PhoneMotionMonitorConfig(
	...
	init {
		...
		require(
			linearAccelerationFullScaleMetersPerSecondSquared.isFinite() && linearAccelerationFullScaleMetersPerSecondSquared > 0.0
		) {
...
class PhoneMotionScoreLogic(
	...
	fun onGyroscopeSample(x: Float, y: Float, z: Float, timestampNanos: Long) {
		...
		if (!magnitude.isFinite() || (previousTimestamp != null && timestampNanos <= previousTimestamp)) {
			return
		}
	...
	private fun combineFreshScores(gyroScore: Double?, linearAccelerationScore: Double?): Double? {
		return when {
			gyroScore != null && linearAccelerationScore != null -> (gyroScore * (1.0 - config.linearAccelerationWeight) + linearAccelerationScore * config.linearAccelerationWeight).coerceIn(
					0.0,
					1.0
				)
