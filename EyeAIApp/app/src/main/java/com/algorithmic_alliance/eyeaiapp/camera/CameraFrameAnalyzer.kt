...
class CameraFrameAnalyzer(
    ...
    private var updatePerformanceText: (String) -> Unit,
    private var setOverlayODResults: (Array<UniffiDetectedObject>) -> Unit,
    private var setOverlayCameraSize: (Size) -> Unit,
    private var updateDebugInputBitmapPreview: (Bitmap) -> Unit,
    private var mediaImageBitmapProvider: () -> Bitmap?
                    ...
                    val inferenceDuration = measureTime {
                        ...
                        withContext(Dispatchers.Main) {
                            if (objects != null) {
                                ...
                                setOverlayCameraSize(Size(frame.width, frame.height))
                            } else {
                                //overlayOD?.reset()
                            }
                        }
    ...
    override fun analyze(image: ImageProxy) {
        if (image.image != null) {
            ...
            val durationMillisFormatted =
                lastCameraFrameDuration.toString(DurationUnit.MILLISECONDS, 2)
