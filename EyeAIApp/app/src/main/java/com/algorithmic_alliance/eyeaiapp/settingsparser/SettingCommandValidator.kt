...
data class SettingParserConfig(
	...
	val frequencySteps: Map<ChangeMagnitude, Int> = mapOf(
		ChangeMagnitude.SMALL to 50, ChangeMagnitude.DEFAULT to 100, ChangeMagnitude.LARGE to 250
	),
	val bpsSteps: Map<ChangeMagnitude, Double> = mapOf(
		ChangeMagnitude.SMALL to 1.0, ChangeMagnitude.DEFAULT to 1.0, ChangeMagnitude.LARGE to 3.0
	),
	val speechSpeedSteps: Map<ChangeMagnitude, Double> = mapOf(
		ChangeMagnitude.SMALL to 0.1, ChangeMagnitude.DEFAULT to 0.2, ChangeMagnitude.LARGE to 0.4
	)
) {
...
class SettingCommandValidator(
	...
	fun validate(command: SettingCommand): SettingCommand {
		...
		if (command.operation !in allowedOperations(command.target)) {
			return withStatus(
				command, SettingParseStatus.NEEDS_CLARIFICATION, "OPERATION_NOT_ALLOWED_FOR_TARGET"
			)
		}
		...
		if (command.target != SettingTarget.SPEAKER && command.speaker != null) {
			return withStatus(
				command, SettingParseStatus.NEEDS_CLARIFICATION, "SPEAKER_NOT_ALLOWED_FOR_TARGET"
			)
		}
		if (command.target == SettingTarget.SPEAKER && command.magnitude != null) {
			return withStatus(
				command, SettingParseStatus.NEEDS_CLARIFICATION, "MAGNITUDE_NOT_ALLOWED_FOR_SPEAKER"
			)
		}
		val expectedUnit = SettingUnitParser.defaultUnit(command.target)
		if (expectedUnit != null && command.unit == null) {
			return withStatus(
				command, SettingParseStatus.INVALID_UNIT, "UNIT_REQUIRED_FOR_NUMERIC_TARGET"
			)
		}
		...
		return if (command.target == SettingTarget.SPEAKER) validateSpeaker(command) else validateNumeric(
			command
		)
	}

	private fun validateSpeaker(command: SettingCommand): SettingCommand =
			...
			SettingOperation.TOGGLE -> {
				if (command.numericValue != null) {
					...
					withStatus(
						...
						"TOGGLE_SPEAKER_MUST_BE_UNSPECIFIED"
					)
				} else {
	...
	private fun validateNumeric(command: SettingCommand): SettingCommand =
		when (command.operation) {
			SettingOperation.SET_ABSOLUTE -> when {
				...
				command.numericValue == null -> withStatus(
					command, SettingParseStatus.NEEDS_VALUE, "NUMERIC_VALUE_REQUIRED"
				)

				!isValidAbsolute(command.target, command.numericValue) -> withStatus(
					command, SettingParseStatus.INVALID_VALUE, "ABSOLUTE_VALUE_OUT_OF_RANGE"
				)

				else -> withStatus(command, SettingParseStatus.COMPLETE)
			}

			SettingOperation.INCREASE, SettingOperation.DECREASE -> when {
				command.numericValue != null && command.magnitude != null -> withStatus(
					command, SettingParseStatus.NEEDS_CLARIFICATION, "NUMERIC_DELTA_AND_MAGNITUDE"
				)

				command.numericValue != null && !isValidDelta(
					command.target, command.numericValue
				...
				command.magnitude in setOf(
					ChangeMagnitude.SMALL, ChangeMagnitude.DEFAULT, ChangeMagnitude.LARGE
				) -> withStatus(command, SettingParseStatus.COMPLETE)

				else -> withStatus(
					command, SettingParseStatus.NEEDS_VALUE, "DELTA_OR_MAGNITUDE_REQUIRED"
				)
			}
	...
	fun isValidAbsolute(target: SettingTarget, value: Double): Boolean = when (target) {
		...
		SettingTarget.BPS -> value.isFinite() && value in config.bpsMin..config.bpsMax && isBpsPrecision(
			value
		)
	...
	private fun allowedOperations(target: SettingTarget): Set<SettingOperation> = when (target) {
		SettingTarget.FREQUENCY, SettingTarget.BPS, SettingTarget.SPEECH_SPEED -> setOf(
		...
		SettingTarget.SPEAKER -> setOf(
			SettingOperation.SET_ABSOLUTE, SettingOperation.TOGGLE, SettingOperation.UNSPECIFIED
		)
	}

	private fun withStatus(
		command: SettingCommand, status: SettingParseStatus, diagnostic: String? = null
