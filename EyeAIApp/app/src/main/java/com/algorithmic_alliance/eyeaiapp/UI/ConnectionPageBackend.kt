	...
	when (deviceCategory) {
		...
		"eye-ai-vision" -> {
			...
			if (selectedDevice != context.getString(R.string.choose_camera_as_input_text)) {
				...
				connectToWifiNetwork(context, selectedDevice, "12345678", onConnected = {
					Log.d(
						LOG_TAG, "[ConnectionPageBackend.connectToDevice] Connection successful"
					)
...
fun connectToWifiNetwork(
	context: Context, ssid: String, password: String, onConnected: () -> Unit, onFailed: () -> Unit
) {
...
data class WifiScanState(
	val networks: List<String>, val rescan: () -> Unit, val awaitScan: suspend () -> List<String>
)

@Composable
fun rememberWifiScanState(
	autoScanOnStart: Boolean = true, setScannState: (Boolean) -> Unit
): WifiScanState {
	...
	suspend fun performScan(): List<String> {
		...
		if (ActivityCompat.checkSelfPermission(
			...
			Log.d(
				LOG_TAG,
				"[ConnectionPageBackend.rememberWifiScanState] Scan failed, missing permission"
			)
			return emptyList()
		}
...
suspend fun scanWifiNetworks(
	context: Context, wifiManager: WifiManager
	...
	val receiver = object : BroadcastReceiver() {
		override fun onReceive(ctx: Context, intent: Intent) {
			...
			Log.d(
				LOG_TAG,
				"[ConnectionPageBackend.scanWifiNetworks] Scan abgeschlossen, ${results.size} Ergebnisse"
			)
