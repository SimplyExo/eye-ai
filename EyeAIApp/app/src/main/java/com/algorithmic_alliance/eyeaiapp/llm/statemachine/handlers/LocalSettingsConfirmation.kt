...
enum class SettingsConfirmationResult {
	APPLIED, REJECTED, NOT_APPLIED, UNKNOWN, FAILED
}
...
class LocalSettingsConfirmation(
	...
	fun evaluate(input: String, currentJson: String?): ConfirmationLabel? {
		val settingsJson = currentJson ?: run {
			trace(
				"[DecisionTrace][ConfirmationModel][RESULT] role=SETTINGS_CONFIRMATION " + "outcome=FAILED reason=MISSING_SETTINGS_CONTEXT " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false"
			)
			return null
		}
		...
		return try {
			...
			trace(
				"[DecisionTrace][ConfirmationModel][RESULT] role=SETTINGS_CONFIRMATION " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + result.toDecisionTraceFields()
			)
			result.label
		} catch (error: Exception) {
			trace(
				"[DecisionTrace][ConfirmationModel][RESULT] role=SETTINGS_CONFIRMATION " + "outcome=FAILED reason=${error::class.simpleName} " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false"
			)
			null
		}
	}
	...
	suspend fun confirmAndApply(
		input: String, currentJson: String?, applySettings: suspend (String) -> Boolean
	...
	suspend fun confirmAndApplyWithResult(
		input: String, currentJson: String?, applySettings: suspend (String) -> SettingsApplyResult
		...
		when (evaluate(input, settingsJson)) {
			ConfirmationLabel.REJECT -> {
				trace(
					"[DecisionTrace][ConfirmationModel][FINAL_ACTION] " + "role=SETTINGS_CONFIRMATION decision=REJECT " + "sideEffect=SKIP_SETTINGS_APPLY apiCalled=false"
				)
			...
			ConfirmationLabel.UNKNOWN -> {
				trace(
					"[DecisionTrace][ConfirmationModel][FINAL_ACTION] " + "role=SETTINGS_CONFIRMATION decision=UNKNOWN " + "sideEffect=NONE clarificationRequired=true apiCalled=false"
				)
				return SettingsConfirmationResult.UNKNOWN
			}

			ConfirmationLabel.ACCEPT -> trace(
				"[DecisionTrace][ConfirmationModel][FINAL_ACTION] " + "role=SETTINGS_CONFIRMATION decision=ACCEPT " + "sideEffect=REQUEST_SETTINGS_APPLY apiCalled=false"
			)
		...
		return when (applySettings(settingsJson)) {
			SettingsApplyResult.APPLIED -> {
				trace(
					"[DecisionTrace][ConfirmationModel][APPLY] " + "role=SETTINGS_CONFIRMATION decision=ACCEPT " + "outcome=APPROVED_AND_APPLIED apiCalled=false"
				)
			...
			SettingsApplyResult.NOT_APPLIED -> {
				trace(
					"[DecisionTrace][ConfirmationModel][APPLY] " + "role=SETTINGS_CONFIRMATION decision=ACCEPT " + "outcome=APPROVED_BUT_NOT_APPLIED apiCalled=false"
				)
			...
			SettingsApplyResult.FAILED -> {
				trace(
					"[DecisionTrace][ConfirmationModel][APPLY] " + "role=SETTINGS_CONFIRMATION decision=ACCEPT " + "outcome=APPROVED_BUT_APPLY_FAILED apiCalled=false"
				)
