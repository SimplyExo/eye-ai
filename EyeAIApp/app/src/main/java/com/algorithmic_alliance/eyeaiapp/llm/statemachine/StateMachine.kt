...
class StateMachine(
	...
	private val speechOutputHandler = SpeechOutputHandler(
		textToSpeechInstance, updateResponseText = setSpeechResponseText
	)
	...
	suspend fun handleIdle(final: String): StateUpdate {
		...
		if (nlpIntent == null) {
			Log.d(
				...
				"[DecisionTrace][StateMachine][ROUTE] classifier=NLP_V2 accepted=false " + "confidence=${
		...
		return when (val settingsRoute = SettingsIntentRouter.route(intentResult)) {
			...
			is SettingsIntentRoute.Direct -> {
				logNlpRoute(
					nlpIntent, "LOCAL_SETTINGS_PARSER", "DIRECT_SETTINGS_PARSE"
				)
	...
	private fun handleUnresolvedCommand(
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][UNRESOLVED] " + "classifierLabel=${classifierLabel?.name ?: "NONE"} semantic=UNRESOLVED_COMMAND " + "externalRequest=false"
		)
		speechOutputHandler.speakAndHandleUi(LocalInteractionMessages.UNRESOLVED_COMMAND)
		return StateUpdate(
			nextState, retainedContext, voskRestartPolicy = VoskRestartPolicy.AUTO_RESTART_AFTER_TTS
		)
	}
	...
	private fun classifyIntentWithNLP(input: String): IntentResult? {
		...
		return try {
			activeModel.classify(input).also { result ->
				...
				Log.d(
					EyeAIApp.APP_LOG_TAG,
					"[DecisionTrace][NLP V2][RESULT] model=${activeModel.info.id} " + "top1=${result.intent} confidence=${
	...
	private suspend fun handleDirectSettingsRequest(
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][HANDOFF] classifier=NLP_V2 " + "intent=${intentResult.intent} evaluator=LOCAL_SETTINGS_PARSER " + "role=SETTINGS_PARAMETER_EXTRACTION " + "settingIntent=${route.settingIntent} " + "originalText='${intentResult.originalText}'"
		)

		return settingsHandler.handleSettingsChoice(
			input = intentResult.originalText, currentJson = settingsContext
		) { newJson ->
	...
	private fun handleObjectDetectionRequest(userInput: String): StateUpdate {
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"Calling ObjectDetectionHandler.handleGermanObjectQuery with: '$germanObjectQuery'"
		)
		...
		when (val result = ObjectDetectionHandler.handleGermanObjectQuery(germanObjectQuery)) {
			...
			is ObjectDetectionHandler.ObjectDetectionResult.ObjectNotFound -> {
				Log.d(
					EyeAIApp.APP_LOG_TAG, "Object NOT FOUND. Available: ${result.availableObjects}"
				)
				speechOutputHandler.speakAndHandleUi(
					"Entschuldigung, das Objekt '$germanObjectQuery' konnte ich nicht finden. " + "Ich sehe aber folgende Objekte: ${
						result.availableObjects.joinToString(
							", "
						)
					}."
				)
	...
	fun handleSettingsMenu(final: String): StateUpdate {
		...
		if (intentResult == null) {
			Log.d(
				EyeAIApp.APP_LOG_TAG,
				"[DecisionTrace][StateMachine][SETTINGS_ROUTE] classifier=NLP_V2 " + "outcome=UNAVAILABLE nextEvaluator=LOCAL_UNRESOLVED role=UNRESOLVED_COMMAND"
			)
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][SETTINGS_ROUTE] classifier=NLP_V2 " + "top1=${evidence.topIntent} top1Confidence=${
				formatPercentage(evidence.topConfidence)
			} " + "bestSettingsIntent=${evidence.bestSettingsIntent} " + "bestSettingsConfidence=${
				formatPercentage(
					evidence.bestSettingsConfidence
				)
			} " + "threshold=${formatPercentage(nlpConfidenceThreshold)} route=${route::class.simpleName} " + "probabilitiesPreserved=true renormalized=false"
		)

		return when (route) {
			is SettingsMenuIntentRoute.LocalSetting -> handleLocalSettingsMenuIntent(route.intent)

			is SettingsMenuIntentRoute.ExternalIntent -> requestExternalIntentContextSwitch(
				intentResult
			)

			SettingsMenuIntentRoute.Abort -> handleCancellation()

			SettingsMenuIntentRoute.AlreadyInSettings -> remindUserSettingsAreAlreadyOpen()

			SettingsMenuIntentRoute.Unresolved -> handleUnresolvedCommand(
				nextState = State.SETTINGS_MENU,
				retainedContext = lastDialogContext,
	...
	private fun requestExternalIntentContextSwitch(
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] action=STORE " + "intent=${intentResult.intent} confidence=${
				formatPercentage(intentResult.confidence)
	...
	private fun remindUserSettingsAreAlreadyOpen(): StateUpdate {
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][SETTINGS_ROUTE] classifier=NLP_V2 " + "intent=OPEN_SETTINGS action=REPLAY_EXISTING_MENU nestedFlowCreated=false"
		)
		speechOutputHandler.speakAndHandleUi(
	...
	private suspend fun executePendingExternalIntent(
		...
		Log.d(
			EyeAIApp.APP_LOG_TAG,
			"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] action=DISPATCH " + "intent=${result.intent} originalText='${result.originalText}' " + "source=STORED_NLP_RESULT nlpInferenceCountForOriginalText=1"
		)
	...
	suspend fun handleSettingsExternalConfirmation(final: String): StateUpdate {
		...
		if (pendingIntent == null) {
			Log.e(
				EyeAIApp.APP_LOG_TAG,
				"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] " + "outcome=INVALID_OR_MISSING action=RETURN_TO_SETTINGS_MENU"
			)
			lastDialogContext = null
			speechOutputHandler.speakAndHandleUi(
		...
		return when (contextSwitchConfirmation.evaluate(final, pendingIntent)) {
			ContextSwitchConfirmationResult.APPROVED -> {
				Log.d(
					EyeAIApp.APP_LOG_TAG,
					"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + "decision=ACCEPT confirmation=APPROVED " + "intent=${pendingIntent.intentResult.intent} " + "action=EXECUTE_STORED_RESULT pendingContextRetained=false " + "nlpReclassified=false"
				)
				lastDialogContext = null
			...
			ContextSwitchConfirmationResult.REJECTED -> {
				Log.d(
					EyeAIApp.APP_LOG_TAG,
					"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + "decision=REJECT confirmation=REJECTED " + "action=STAY_IN_SETTINGS_MENU nextState=SETTINGS_MENU " + "pendingExternalIntent=cleared"
				)
				lastDialogContext = null
				speechOutputHandler.speakAndHandleUi(
			...
			ContextSwitchConfirmationResult.UNKNOWN -> {
				Log.d(
					EyeAIApp.APP_LOG_TAG,
					"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + "decision=UNKNOWN confirmation=UNKNOWN action=REQUEST_CLARIFICATION " + "nextState=SETTINGS_EXTERNAL_CONFIRMATION pendingContextRetained=true"
				)
				speechOutputHandler.speakAndHandleUi(
			...
			ContextSwitchConfirmationResult.FAILED -> {
				Log.w(
					EyeAIApp.APP_LOG_TAG,
					"[DecisionTrace][StateMachine][PENDING_EXTERNAL_INTENT] " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + "decision=FAILED confirmation=FAILED action=KEEP_PENDING_CONTEXT " + "nextState=SETTINGS_EXTERNAL_CONFIRMATION pendingContextRetained=true"
				)
				StateUpdate(State.SETTINGS_EXTERNAL_CONFIRMATION, lastDialogContext)
			}
		}
