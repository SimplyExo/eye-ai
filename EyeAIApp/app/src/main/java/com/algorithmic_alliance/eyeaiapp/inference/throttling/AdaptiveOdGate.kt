...
class AdaptiveOdGate(
	...
	private fun refreshMode(now: Long) {
		...
		val strongVisual = visualSignal?.takeIf {
			visual != null && it.score >= ObjectDetectionPolicy.BURST_VISUAL_ENTRY && it.atNanos > (lastBurstSignalAtNanos
				?: Long.MIN_VALUE)
		}
	...
	private fun hasActiveEntry(visual: Double?, motion: Double?): Boolean =
		(visual != null && visual >= ObjectDetectionPolicy.ACTIVE_VISUAL_ENTRY) || (motion != null && motion >= ObjectDetectionPolicy.ACTIVE_MOTION_ENTRY)

	private fun hasActiveExit(visual: Double?, motion: Double?): Boolean =
		(visual != null && visual >= ObjectDetectionPolicy.ACTIVE_VISUAL_EXIT) || (motion != null && motion >= ObjectDetectionPolicy.ACTIVE_MOTION_EXIT)

	private fun isLowActivity(visual: Double?, motion: Double?): Boolean =
		(visual == null || visual <= ObjectDetectionPolicy.QUIET_VISUAL) && (motion == null || motion <= ObjectDetectionPolicy.QUIET_MOTION)
