...
sealed interface LocalSettingsCommandExecution {
    data class Ready(
        val command: SettingCommand, val resolution: SettingResolution, val settingsJson: String
    ) : LocalSettingsCommandExecution

    data class NotReady(
        val command: SettingCommand, val resolution: SettingResolution
    ) : LocalSettingsCommandExecution

    data class UnsupportedAppRepresentation(
        val command: SettingCommand, val resolution: SettingResolution, val diagnostic: String
    ) : LocalSettingsCommandExecution
}
...
class SettingsCommandExecutor(
    ...
    fun execute(
        command: SettingCommand, current: CurrentSettingsState
    ): LocalSettingsCommandExecution {
        ...
        val change = toChangedSetting(resolution)
            ?: return LocalSettingsCommandExecution.UnsupportedAppRepresentation(
                command, resolution, "ANDROID_BPS_REQUIRES_INTEGER"
            )
        return LocalSettingsCommandExecution.Ready(
            command = command, resolution = resolution, settingsJson = JSONObject().apply {
    ...
    private fun toChangedSetting(resolution: SettingResolution): JSONObject? =
        when (resolution.target) {
            ...
            SettingTarget.SPEECH_SPEED -> numericChange(
                "tts_speed", resolution, requireWhole = false
            )

            SettingTarget.SPEAKER -> {
                val speaker = (resolution.requestedValue as? ResolvedSettingValue.Speaker)?.value
                    ?: return null
    ...
    private fun numericChange(
        key: String, resolution: SettingResolution, requireWhole: Boolean
    ): JSONObject? {
        val value =
            (resolution.requestedValue as? ResolvedSettingValue.Numeric)?.value ?: return null
