...
class ContextSwitchConfirmation(
	...
	fun evaluate(
		input: String, pendingIntent: PendingExternalIntent
		...
		val result = try {
			...
			trace(
				"[DecisionTrace][ConfirmationModel][RESULT] " + "role=SETTINGS_CONTEXT_SWITCH_CONFIRMATION outcome=FAILED " + "reason=${error::class.simpleName} evaluator=LOCAL_CONFIRMATION_MODEL " + "apiCalled=false"
			)
		...
		trace(
			"[DecisionTrace][ConfirmationModel][RESULT] " + "role=SETTINGS_CONTEXT_SWITCH_CONFIRMATION outcome=$outcome " + "evaluator=LOCAL_CONFIRMATION_MODEL apiCalled=false " + result.toDecisionTraceFields()
		)
		return outcome
	}
}
