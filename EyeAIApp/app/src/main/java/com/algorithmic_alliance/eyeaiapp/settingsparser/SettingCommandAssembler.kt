...
class SettingCommandAssembler {
	fun assemble(
		...
		val numericValue = when (numbers.status) {
			NumberNormalizationStatus.SUCCESS -> normalizedNumericValueForOperation(
				numbers = numbers, operation = operation.operation
			).also { selected ->
		...
		val status = initialStatus(
			target, operation.operation, numericValue, relevantSpeaker, unit, diagnostics
		)
		...
		return SettingCommand(
			...
			speakerProbabilities = relevantSpeaker?.probabilities?.copyOf() ?: FloatArray(
	...
	private fun normalizedNumericValueForOperation(
		numbers: NumberNormalizationResult, operation: SettingOperation
	): Double {
		val raw = numbers.values.single()
		val occurrence = numbers.occurrences.singleOrNull {
			it.status == NumberOccurrenceStatus.SUCCESS
		}
		return if (operation == SettingOperation.DECREASE && raw < 0.0 && occurrence != null && lexicalMinusPrefix.containsMatchIn(
	...
	private fun initialStatus(
		...
		target == SettingTarget.SPEAKER && operation == SettingOperation.SET_ABSOLUTE && (speaker == null || speaker.speaker == SpeakerChoice.UNSPECIFIED) -> SettingParseStatus.NEEDS_VALUE

		target == SettingTarget.SPEAKER && operation in setOf(
			SettingOperation.SET_ABSOLUTE, SettingOperation.TOGGLE
		...
		operation in setOf(
			SettingOperation.SET_ABSOLUTE, SettingOperation.INCREASE, SettingOperation.DECREASE
		) -> SettingParseStatus.COMPLETE
